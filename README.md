...
```sh
npm pack                                               # 产出 dsh-session-delete-0.2.2.tgz
dsh plugin --profile web add ./dsh-session-delete-0.2.2.tgz
```
...
4. 删除 / 清空时会尽量同步清掉 `workspace.json` 里的 `archivedSessionIds` ghost，避免官方归档计数虚高
